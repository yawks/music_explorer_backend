...
def test_dumps():
    assert ObjectIdManager.instance().dumps(SpotifyId(
...
def test_dumps_list():
    ...
    assert ObjectIdManager.instance().dumps_list(object_ids) == "eyJzcCI6ICIxMjM0IiwgInl0IjogIkFCQ0QifQ=="


def test_loads():
    object_ids: ObjectIds = ObjectIdManager.instance().loads(
